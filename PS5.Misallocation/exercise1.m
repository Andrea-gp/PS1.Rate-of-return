%The mean is 0 because ln(1)=0 as we are working in logarithms all the time
s=1000;mu=[0,0];sigma=[1,0;0,1];
ln_k=randn(s,1);
ln_z=randn(s,1);
%Plot in logs:
[X1,X2] = meshgrid(ln_k,ln_z);
X = [X1(:) X2(:)];
mu=[1,1];sigm=[1,0;0,1];
p = mvnpdf(X,mu,sigma);
Z = reshape(p,length(ln_k),length(ln_z));
figure(1)
surf(X1,X2,Z)
title('Plot in logs')
xlabel('ln(k)')
ylabel('ln(z)')
zlabel('Joint Density Function')
%Plot in levels:
k=exp(randn(s,1));
z=exp(randn(s,1));
[X1,X2] = meshgrid(k,z);
X = [X1(:) X2(:)];
p = mvnpdf(X,mu,sigma);
Z = reshape(p,length(k),length(z));
figure(2)
surf(X1,X2,Z)
title('Plot in levels')
xlabel('k')
ylabel('z')
zlabel('Joint Density Function')
%%
